...
export default deployScript(
  async (env: Environment) => {
    ...
    const rigoToken = await env.deploy("RigoToken", {
      account: deployer,
    ...
    const oracle = await env.deploy("MockOracle", {
      account: deployer,
      artifact: await readArtifact("MockOracle"),
      args: []
      }, {deterministic: true});

    const weth = await env.deploy("WETH9", {
      account: deployer,
      artifact: await readArtifact("WETH9"),
      args: []
      }, {deterministic: true});

    await env.deploy("MockUniswapNpm", {
      account: deployer,
    ...
    const permit2 = await env.deploy("MockPermit2", {
      account: deployer,
      artifact: await readArtifact("MockPermit2"),
      args: []
      }, {deterministic: true});

    const univ4Posm = await env.deploy("MockUniswapPosm", {
      account: deployer,
    ...
    const acrossSpokePool = await env.deploy("MockAcrossSpokePool", {
      account: deployer,
      artifact: await readArtifact("MockAcrossSpokePool"),
      args: [weth.address]
      }, {deterministic: true});

    await env.deploy("MockAcrossMulticallHandler", {
      account: deployer,
    ...
    const mockTokenJar = await env.deploy("MockTokenJar", {
      account: deployer,
    ...
    const poolImplementation = await env.deploy("SmartPool", {
      ...
      args: [env.get("Authority").address, extensionsMapAddress, mockTokenJar.address]
      }, {deterministic: true});
    ...
    const aStaking = await env.deploy("AStaking", {
      account: deployer,
      artifact: await readArtifact("AStaking"),
      args: [stakingProxy.address, rigoToken.address, grgTransferProxy.address]
      }, {deterministic: true});
    ...
    // same on altchains but different from one deployed on Ethereum
    const inflation = await env.deploy("Inflation", {
      account: deployer,
    ...
    const aIntents = await env.deploy("AIntents", {
      account: deployer,
