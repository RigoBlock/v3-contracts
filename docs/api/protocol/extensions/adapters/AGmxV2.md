...
```solidity
function claimFundingFees(address[] markets, address[] tokens, address) external
```
...
```solidity
function claimCollateral(address[] markets, address[] tokens, uint256[] timeKeys, address) external
```
