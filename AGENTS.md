...
22. **TEST FAILURES AFTER AUDIT FIXES ARE SIGNALS, NOT NOISE** — When an audit fix causes a previously-passing test to fail, **NEVER remove or rewrite the test** to make the suite green again without first understanding why it failed. A failing test may be protecting exactly the scenario the audit fix inadvertently introduces or closes off. Required process: (1) identify what the test was asserting, (2) establish why the fix changes that behavior, (3) determine whether the test is now invalid OR whether the fix is wrong. **RIGO-14 canonical example**: removing `payable` from `fallback()` made `test_Donate_WithMsgValue_Reverts` fail for a new `reason — investigating that failure revealed the fix was wrong (it would block legitimate ETH forwarding). The test was protecting the very thing the fix silently broke.
