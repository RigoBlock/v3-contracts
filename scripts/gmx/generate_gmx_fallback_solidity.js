...
function buildTestFile() {
  ...
  lines.push(
    ...
    "",
    "    // =========================================================================",
    "    // Staleness bound: the bound exceeds the largest guarded feed heartbeat",
    "    //  (SPY/USD: 24h), so publication jitter on ordinary heartbeat rounds must",
    "    //  not false-positive as staleness; genuinely stale feeds must still return",
    "    //  the zero price.",
    "    // =========================================================================",
    "",
    "    /// @notice A feed `updatedAt` just past 24h (worst measured heartbeat jitter: +25s)",
    "    ///  but within the 26h bound must still return a valid price (SPY regression).",
    "    function test_StalePast24hWithinBound_ReturnsValidPrice() public {",
    "        FallbackEntry memory e = entries[0];",
    "        vm.warp(1_700_000_000);",
    "        int256 answer = 123_456_789;",
    "        _mockLatestRoundData(e.feed, answer, block.timestamp - 24 hours - 25 seconds);",
    "",
    "        Price.Props memory price = GmxFallback.getFallbackPrice(e.token);",
    "",
    "        uint256 expected = (uint256(answer) * 10 ** e.exponent) / _FLOAT_PRECISION;",
    "        assertEq(price.min, expected);",
    "        assertEq(price.max, expected);",
    "    }",
    "",
    "    /// @notice A feed genuinely past the 26h staleness bound must return the zero price.",
    "    function test_StalePastBound_ReturnsZeroPrice() public {",
    "        FallbackEntry memory e = entries[0];",
    "        vm.warp(1_700_000_000);",
    "        _mockLatestRoundData(e.feed, 123_456_789, block.timestamp - 26 hours - 1 hours);",
    "",
    "        Price.Props memory price = GmxFallback.getFallbackPrice(e.token);",
    "",
    ...
    "    }",
    "",
    "    function _mockLatestRoundData(address feed, int256 answer, uint256 updatedAt) internal {",
    "        vm.mockCall(",
    "            feed,",
