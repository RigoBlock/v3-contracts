...
describe("Proxy", async () => {
  ...
  const setupTests = deployments.createFixture(async ({ deployments }) => {
    ...
    const UniswapV4Posm =
      await hre.ethers.getContractFactory("MockUniswapPosm");
  ...
  describe("receive", async () => {
    ...
    it("should receive ether", async () => {
      ...
      expect(await hre.ethers.provider.getBalance(pool.address)).to.be.deep.eq(
        etherAmount,
      );
    });
  });
  ...
  describe("erc20", async () => {
    it("should transfer tokens", async () => {
      ...
      await expect(pool.transfer(user2.address, balance))
        .to.emit(pool, "Transfer")
        .withArgs(user1.address, user2.address, balance);
      expect(await pool.balanceOf(user1.address)).to.be.eq(0);
      expect(await pool.balanceOf(user2.address)).to.be.eq(balance);
    });
    ...
    it("should transferFrom with approval", async () => {
      ...
      await pool.mint(user1.address, etherAmount, 0, { value: etherAmount });
      const balance = await pool.balanceOf(user1.address);
      await pool.approve(user2.address, balance);
      expect(await pool.allowance(user1.address, user2.address)).to.be.eq(
        balance,
      );
      await expect(
        pool.connect(user2).transferFrom(user1.address, user2.address, balance),
      )
        .to.emit(pool, "Transfer")
        .withArgs(user1.address, user2.address, balance);
      expect(await pool.balanceOf(user1.address)).to.be.eq(0);
    ...
    it("should approve and update allowance", async () => {
      ...
      await expect(pool.approve(user2.address, amount))
    ...
    it("should revert transfer to zero address", async () => {
      ...
      await pool.mint(user1.address, etherAmount, 0, { value: etherAmount });
      await expect(
        pool.transfer(AddressZero, await pool.balanceOf(user1.address)),
      ).to.be.revertedWith("PoolTokenTransferToNullAddress");
    });

    it("should revert transfer to pool itself", async () => {
      const { pool } = await setupTests();
      const etherAmount = parseEther("1");
      await pool.mint(user1.address, etherAmount, 0, { value: etherAmount });
      await expect(
        pool.transfer(pool.address, await pool.balanceOf(user1.address)),
      ).to.be.revertedWith("PoolTokenCannotReceivePoolTokens");
    });

    it("should revert transfer to another registered pool", async () => {
      ...
      await factory.createPool("pool2", "POOL2", AddressZero);
      await expect(
        pool.transfer(newPoolAddress, await pool.balanceOf(user1.address)),
      ).to.be.revertedWith("PoolTokenCannotReceivePoolTokens");
    });

    it("should revert transfer with insufficient balance", async () => {
      const { pool } = await setupTests();
      await pool.mint(user1.address, parseEther("1"), 0, {
        value: parseEther("1"),
      });
      await expect(
        pool.transfer(user2.address, parseEther("2")),
      ).to.be.revertedWith("PoolTokenInsufficientBalance");
    });
    ...
    it("should revert transferFrom with insufficient allowance", async () => {
      ...
      await pool.approve(user2.address, parseEther("0.5"));
      await expect(
        pool
          .connect(user2)
          .transferFrom(user1.address, user2.address, parseEther("1")),
      ).to.be.revertedWith("PoolTokenInsufficientAllowance");
    });

    it("should inherit sender activation on transfer", async () => {
      const { pool } = await setupTests();
      const etherAmount = parseEther("1");
      await pool.mint(user1.address, etherAmount, 0, { value: etherAmount });
      const senderAccountBefore = await pool.getUserAccount(user1.address);
      await pool.transfer(user2.address, await pool.balanceOf(user1.address));
      const recipientAccount = await pool.getUserAccount(user2.address);
      expect(recipientAccount.activation).to.be.eq(
        senderAccountBefore.activation,
      );
    });

    it("should not extend recipient activation on transfer", async () => {
      const { pool } = await setupTests();
      // User1 mints first (earlier activation)
      await pool.mint(user1.address, parseEther("1"), 0, {
        value: parseEther("1"),
      });
      const senderAccountBefore = await pool.getUserAccount(user1.address);

      // User2 mints after a block advance, giving them a stricter (later) activation
      await waffle.provider.send("evm_mine", []);
      await pool
        .connect(user2)
        .mint(user2.address, parseEther("1"), 0, { value: parseEther("1") });

      // Transfer user1's balance to user2; user2 must inherit user1's earlier activation
      await pool.transfer(user2.address, await pool.balanceOf(user1.address));

      const user2AccountAfter = await pool.getUserAccount(user2.address);
      expect(user2AccountAfter.activation).to.be.eq(
        senderAccountBefore.activation,
      );
    });

    it("should not change total supply on transfer", async () => {
      const { pool } = await setupTests();
      await pool.mint(user1.address, parseEther("1"), 0, {
        value: parseEther("1"),
      });
      ...
      await pool.transfer(user2.address, await pool.balanceOf(user1.address));
      expect(await pool.totalSupply()).to.be.eq(totalSupplyBefore);
    });
  });

  describe("poolTokenGuard", async () => {
    it("should revert accepting a registered pool token as mint token", async () => {
      const { factory, pool } = await setupTests();
      // Create a second pool to use as tokenIn
      const { newPoolAddress } = await factory.callStatic.createPool(
        "pool2",
        "POOL2",
        AddressZero,
      ...
      await factory.createPool("pool2", "POOL2", AddressZero);
      const pool2 = await hre.ethers.getContractAt("SmartPool", newPoolAddress);
      // Owner tries to accept the pool token as a mint token.
      await expect(
        pool.setAcceptableMintToken(pool2.address, true),
      ).to.be.revertedWith("PoolTokenNotAllowed");
    });

    it("should revert setAcceptableMintToken for a registered pool token", async () => {
      const { factory, pool } = await setupTests();
      const { newPoolAddress } = await factory.callStatic.createPool(
        "pool2",
        "POOL2",
      ...
      await factory.createPool("pool2", "POOL2", AddressZero);
      await expect(
        pool.setAcceptableMintToken(newPoolAddress, true),
      ).to.be.revertedWith("PoolTokenNotAllowed");
    });
  });
  ...
  describe("setTransactionFee", async () => {
    ...
    it("should not set fee if caller not owner", async () => {
      ...
      await expect(pool.setTransactionFee(2)).to.be.revertedWith(
    ...
    it("should not set fee higher than 1 percent", async () => {
      ...
      await expect(pool.setTransactionFee(101))
        .to.be.revertedWith("PoolFeeBiggerThanMax")
        .withArgs(100);
    });
  });
  ...
  describe("setOwner", async () => {
    it("should revert if caller not owner", async () => {
      const { pool } = await setupTests();
      await expect(
        pool.connect(user2).setOwner(user2.address),
      ).to.be.revertedWith("PoolCallerIsNotOwner");
    });

    it("should revert if new owner null address", async () => {
      const { pool } = await setupTests();
      await expect(pool.setOwner(AddressZero)).to.be.revertedWith(
        "PoolNullOwnerInput",
      );
    });
  ...
  describe("mint", async () => {
    it("should create new tokens", async () => {
      ...
      const amount = await pool.callStatic.mint(user1.address, etherAmount, 0, {
        value: etherAmount,
      });
      await expect(
      ...
      const expectedMintedAmount = etherAmount.sub(
        etherAmount.mul(spread).div(10000),
      );
      await expect(
        pool.mint(user1.address, etherAmount, 0, { value: etherAmount }),
      )
        .to.emit(pool, "Transfer")
        .withArgs(AddressZero, user1.address, expectedMintedAmount);
    ...
    it("should revert with invalid recipient", async () => {
      ...
      await expect(
        pool.mint(AddressZero, etherAmount, 0, { value: etherAmount }),
    ...
    it("should revert if user not whitelisted when whitelist enabled", async () => {
      ...
      const mintedAmount = await pool.callStatic.mint(
        recipient,
        etherAmount,
        0,
        { value: etherAmount },
      ...
      await expect(pool.mint(recipient, etherAmount, 0, { value: etherAmount }))
        .to.emit(pool, "Transfer")
        .withArgs(AddressZero, recipient, mintedAmount);
    });
    ...
    it("should allocate fee tokens to fee recipient", async () => {
      ...
      let mintedAmount = await pool.callStatic.mint(
        user1.address,
        etherAmount,
        0,
        { value: etherAmount },
      ...
      await expect(
        ...
        .to.emit(pool, "Transfer")
        .withArgs(AddressZero, feeCollector, mintedAmount);
      ...
      mintedAmount = await pool
        .connect(user2)
        .callStatic.mint(user2.address, etherAmount, 0, { value: etherAmount });
    ...
    it("should read from storage with previously burnt supply", async () => {
      ...
      expect(ethBalance).to.be.eq(
        etherAmount.mul(5).sub(spread.mul(etherAmount).div(10000)),
      );
  ...
  describe("burn", async () => {
    it("should burn tokens", async () => {
      ...
      await timeTravel({ seconds: 2592000, mine: true });
      const spread = BigNumber.from((await pool.getPoolParams()).spread);
      const netAmount = etherAmount.sub(etherAmount.mul(spread).div(10000));
      const poolBalance = await hre.ethers.provider.getBalance(pool.address);
      ...
      expect(netRevenue).to.be.eq(
        poolBalance.sub(poolBalance.mul(spread).div(10000)),
      );
      // the following is true with fee set as 0
      await expect(pool.burn(userPoolBalance, 1))
        .to.emit(pool, "Transfer")
        .withArgs(user1.address, AddressZero, userPoolBalance);
      expect(await hre.ethers.provider.getBalance(pool.address)).to.be.deep.eq(
        0,
      );
    ...
    it("should not allow dos by frontrun with mint to holder", async () => {
      ...
      await expect(
        pool
          .connect(user2)
          .mint(user1.address, etherAmount, 0, { value: etherAmount }),
      ).to.be.revertedWith("InvalidOperator");
      // the following is true with fee set as 0
      await expect(pool.burn(userPoolBalance, 1))
        .to.emit(pool, "Transfer")
    ...
    it("should allocate fee tokens to fee recipient", async () => {
      ...
      await expect(pool.burn(userPoolBalance.div(2), 1))
        .to.emit(pool, "Transfer")
        .withArgs(user1.address, AddressZero, userPoolBalance.div(2));
      ...
      await expect(pool.burn(userPoolBalance, 1))
        ...
        .withArgs(user1.address, AddressZero, burntAmount);
    });
    ...
    it("should not allow dos by setting holder as fee recipient", async () => {
      ...
      await pool
        .connect(user3)
        .mint(user3.address, etherAmount, 0, { value: etherAmount });
      await timeTravel({ seconds: 2592000, mine: true });
      const transactionFee = 50;
      await pool.setTransactionFee(transactionFee);
      ...
      await expect(pool.changeFeeCollector(user3.address)).to.be.revertedWith(
        "InvalidOperator",
      );
      ...
      await expect(
        pool
          .connect(user3)
          .mint(user2.address, etherAmount, 0, { value: etherAmount }),
      ).to.be.revertedWith("InvalidOperator");
      await pool
        .connect(user2)
        .mint(user2.address, etherAmount, 0, { value: etherAmount });
      const fee = userPoolBalance.div(10000).mul(transactionFee);
      const burntAmount = userPoolBalance.sub(fee);
      await expect(pool.connect(user3).burn(userPoolBalance, 1))
        .to.emit(pool, "Transfer")
        .withArgs(user3.address, user1.address, fee)
        .and.to.emit(pool, "Transfer")
        .withArgs(user3.address, AddressZero, burntAmount);
    });
  });
  ...
  it("should revert without enough base token balance", async () => {
    ...
    await timeTravel({ seconds: 2592000, mine: true });
    await expect(pool.burn(parseEther("6"), 1)).to.be.revertedWith(
  ...
  describe("updateUnitaryValue", async () => {
    ...
    it("should handle previously burnt supply gracefully", async () => {
      ...
      expect(ethBalance).to.be.eq(
        etherAmount.sub(etherAmount.mul(spread).div(10000)),
      );
      await timeTravel({ seconds: 2592000, mine: true });
  ...
  describe("setKycProvider", async () => {
    ...
    it("should set pool kyc provider", async () => {
      ...
      await expect(pool.setKycProvider(user2.address)).to.be.revertedWith(
    ...
    it("should allow reset kyc provider", async () => {
      const { pool } = await setupTests();
      expect((await pool.getPoolParams()).kycProvider).to.be.eq(AddressZero);
      await expect(pool.setKycProvider(AddressZero)).to.be.revertedWith(
  ...
  describe("changeFeeCollector", async () => {
    ...
    it("should revert if fee collector has not given permission to pool operator", async () => {
      const { pool } = await setupTests();
      await expect(pool.changeFeeCollector(AddressZero)).to.be.revertedWith(
        "InvalidOperator",
      );
    });
    ...
    it("should set fee collector", async () => {
      ...
      await expect(pool.changeFeeCollector(user2.address))
        ...
        .withArgs(user1.address, pool.address, user2.address);
    ...
    it("should revert if new is same as current", async () => {
      ...
      await expect(pool.changeFeeCollector(initialCollector))
        .to.emit(pool, "NewCollector")
        .withArgs(user1.address, pool.address, initialCollector);
    });
  });
  ...
  describe("changeSpread", async () => {
    it("should revert if caller not pool owner", async () => {
      const { pool } = await setupTests();
      await expect(pool.connect(user2).changeSpread(1)).to.be.revertedWith(
        "PoolCallerIsNotOwner",
      );
    });
    ...
    it("should revert with rogue values", async () => {
      ...
      await expect(pool.changeSpread(0))
        .to.be.revertedWith("PoolSpreadInvalid")
        .withArgs(500);
    ...
    it("should revert if same as current", async () => {
      ...
      await expect(pool.changeSpread(10)).to.be.revertedWith(
        "OwnerActionInputIsSameAsCurrent",
      );
      await expect(pool.changeSpread(400))
        .to.emit(pool, "SpreadChanged")
        .withArgs(pool.address, 400);
  ...
  describe("changeMinPeriod", async () => {
    it("should revert if caller not pool owner", async () => {
      const { pool } = await setupTests();
      await expect(pool.connect(user2).changeMinPeriod(1)).to.be.revertedWith(
        "PoolCallerIsNotOwner",
      );
    });
    ...
    it("should revert with rogue values", async () => {
      ...
      await expect(pool.changeMinPeriod(1))
        .to.be.revertedWith("PoolLockupPeriodInvalid")
        .withArgs(86400, 2592000);
      // max 30 days lockup
    ...
    it("should change spread", async () => {
      ...
      await expect(pool.changeMinPeriod(newPeriod))
        ...
        .withArgs(pool.address, newPeriod);
    ...
    it("will revert if spread same as current", async () => {
      ...
      await expect(pool.changeMinPeriod(newPeriod))
        ...
        .withArgs(pool.address, newPeriod);
      expect((await pool.getPoolParams()).minPeriod).to.be.eq(newPeriod);
      await expect(pool.changeMinPeriod(newPeriod)).to.be.revertedWith(
        "OwnerActionInputIsSameAsCurrent",
      );
      expect((await pool.getPoolParams()).minPeriod).to.be.eq(newPeriod);
    });
