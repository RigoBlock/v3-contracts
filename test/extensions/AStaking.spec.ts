...
describe("AStaking", async () => {
    ...
    describe("withdraw rewards", async () => {
        it('withdraw delegator rewards', async () => {
            ...
            // TODO: check if should finalize inside withdrawdelegatorrewars call (will not do anything if already finalized)
            const poolId = await stakingProxy.poolIdByRbPoolAccount(newPoolAddress)
            await stakingProxy.finalizePool(poolId)
            // TODO: calculate delegator reward amount and assert value in following log
