...
contract AGmxV2ForkTest is Test {
    ...
    uint256 private constant LIT_COLLATERAL_AMOUNT_USDC = 4_000 * 1e6;
    ...
    function setUp() public {
        ...
        bytes32 salt = keccak256(abi.encodePacked("GMX_V2_FORK_TEST_V2", block.chainid));
        ...
        // ------------------------------------------------------------------
        // 6. Fund pool with WETH and USDC (covers collateral + fees for long/short)
        // ------------------------------------------------------------------
        deal(ARB_WETH, pool, 10 ether);
        deal(ARB_USDC, pool, 20_000e6);
    }
    ...
    function test_CreateIncreaseOrder_CallbackDisabledAndSavedCallbackSet() public {
        ...
        bytes32 savedCallbackKey = keccak256(
            abi.encode(keccak256(abi.encode("SAVED_CALLBACK_CONTRACT")), pool, p.addresses.market)
        );
    ...
    function test_ClaimFundingFees_PartialDecreaseFlushesFundingFees() public {
        ...
        // Seed claimable funding deterministically instead of relying on the market's organic
        // funding rate at the pinned block, which can be zero or pay the opposite side (the
        // accrual is block-dependent and made this test fail after ARB_BLOCK was bumped).
        // GMX settles claimable funding on decrease as
        // (latestClaimableFundingAmountPerSize - position.cached) * sizeInUsd / 1e30
        // (PositionPricingUtils.getFundingFees), so bumping the CLAIMABLE_FUNDING_AMOUNT_PER_SIZE
        // accumulator by `delta` flushes ~delta * sizeInUsd / 1e30, i.e. ~0.5 WETH here.
        bytes32 prefix = keccak256(abi.encode("CLAIMABLE_FUNDING_AMOUNT_PER_SIZE"));
        bytes32 key = keccak256(abi.encode(prefix, GMX_ETH_USD_MARKET, ARB_WETH, true));
        uint256 current = IDataStore(GMX_DATA_STORE).getUint(key);
        uint256 perSizeDelta = (0.5e18 * 1e30) / SIZE_DELTA_USD;
        // DataStore stores uintValues[key] at keccak256(abi.encode(key, 0)) (slot 0 mapping).
        bytes32 slot = keccak256(abi.encode(key, uint256(0)));
        vm.store(GMX_DATA_STORE, slot, bytes32(current + perSizeDelta));
        assertEq(IDataStore(GMX_DATA_STORE).getUint(key), current + perSizeDelta, "seed failed");
    ...
    function test_ClaimCollateral_FormulaParity_RealTransfer() public {
        address token = ARB_WETH;
        ...
        for (uint256 i; i < apps.length; ++i) {
            if (uint256(apps[i].appType) == uint256(Applications.GMX_V2_POSITIONS)) {
                for (uint256 j; j < apps[i].balances.length; ++j) {
                    if (apps[i].balances[j].token == token && uint256(apps[i].balances[j].amount) == amount) {
                        found = true;
                        break;
                    }
                }
        ...
        assertTrue(found, "EApps must report the claimable collateral balance");
        ...
        assertEq(balanceAfter - balanceBefore, amount, "actual WETH transfer must equal computed claimable amount");
    }

    /// @dev Helper for `test_ClaimCollateral_FormulaParity_RealTransfer`.
    ///  Creates a synthetic GMX market token, writes claimable-collateral DataStore keys
    ///  for `pool`, and records the key in callback storage by simulating a GMX
    ///  afterOrderExecution callback.
    function _setupClaimableCollateral(
        address token,
        uint256 amount
    ) private returns (address market, uint256 timeKey) {
        // Use a synthetic market token registered in GMX's DataStore. This lets the
        // claimCollateral validation pass with a small WETH balance while still exercising
        // the real GMX Router and MarketUtils code paths.
        MockGmxMarketToken marketToken = new MockGmxMarketToken();
        ...
        // collateral claimable. This works even when the delay is larger than the divisor.
        vm.warp(timeKey * timeDivisor + 1);

        _registerMarketInDataStore(market, token, token, ARB_USDC);
        _writeClaimableCollateralDataStore(market, token, timeKey, amount);

        // Ensure the synthetic market token has enough WETH to pay out the claim.
        deal(token, market, amount * 2);

        // Simulate the callback that records the key in pool storage (as would happen on liquidation/ADL).
        _simulateGmxCallback(market);

        // Warp past the claim delay so the time-vesting fallback applies.
        vm.warp(timeKey * timeDivisor + claimDelay + 1);
    }

    /// @dev Writes the account-specific and market-wide claimable-collateral DataStore keys.
    function _writeClaimableCollateralDataStore(
        address market,
        address token,
        uint256 timeKey,
        uint256 amount
    ) private {
        bytes32 accountAmountKey = keccak256(
            abi.encode(GmxCallbackLib.CLAIMABLE_COLLATERAL_AMOUNT_KEY, market, token, timeKey, pool)
        );
        bytes32 marketAmountKey = keccak256(abi.encode(GmxCallbackLib.CLAIMABLE_COLLATERAL_AMOUNT_KEY, market, token));
    ...
    /// @dev Registers a synthetic market in GMX's DataStore so `Reader.getMarket` and
    ///  `validateMarketTokenBalance` treat it as an enabled market.
    function _registerMarketInDataStore(
        address market,
        address indexToken,
        address longToken,
        address shortToken
    ) private {
        bytes32 marketListKey = keccak256(abi.encode("MARKET_LIST"));
        bytes32 marketTokenField = keccak256(abi.encode("MARKET_TOKEN"));
        bytes32 indexTokenField = keccak256(abi.encode("INDEX_TOKEN"));
        bytes32 longTokenField = keccak256(abi.encode("LONG_TOKEN"));
        bytes32 shortTokenField = keccak256(abi.encode("SHORT_TOKEN"));

        IDataStore ds = IDataStore(GMX_DATA_STORE);
        address controller = _getController();
        vm.startPrank(controller);
        ds.addAddress(marketListKey, market);
        ds.setAddress(keccak256(abi.encode(market, marketTokenField)), market);
        ds.setAddress(keccak256(abi.encode(market, indexTokenField)), indexToken);
        ds.setAddress(keccak256(abi.encode(market, longTokenField)), longToken);
        ds.setAddress(keccak256(abi.encode(market, shortTokenField)), shortToken);
        vm.stopPrank();
    }
    ...
    function test_ENavView_NavMatchesStoredNav_WithOpenPosition_PosAndNegPnL() public {
        ...
        assertGt(
            storedNavHighPrice,
            storedNavLowPrice,
            "long position: high price must give higher NAV than low price"
        );
    }

    // =========================================================================
    // Tests — synthetic index token (LIT/USD) via hardcoded Chainlink fallback feed
    // =========================================================================
    ...
    // =========================================================================
    // Helpers — LIT/USD synthetic index positions and keeper execution
    // =========================================================================
    ...
    /// @dev Etches a minimal bytecode over the LIT fallback aggregator before vm.mockCall
    ///  intercepts latestRoundData(), so Foundry does not hit "call to non-contract address"
    ///  on any code path that reads the feed outside the mocked selector. Then provides a
    ///  stable baseline ($3.496 / LIT) for order creation and baseline valuation. Kept out
    ///  of setUp on purpose: the fallback-feed fork tests below read the REAL LIT feed.
    function _setupLitFallback() private {
        ...
        return
            IBaseOrderUtils.CreateOrderParams({
                ...
                numbers: IBaseOrderUtils.CreateOrderParamsNumbers({
                    ...
                    initialCollateralDeltaAmount: LIT_COLLATERAL_AMOUNT_WETH,
                    triggerPrice: 0,
    ...
    function _litShortParams() private pure returns (IBaseOrderUtils.CreateOrderParams memory) {
        return
            IBaseOrderUtils.CreateOrderParams({
                ...
                numbers: IBaseOrderUtils.CreateOrderParamsNumbers({
                    sizeDeltaUsd: LIT_SIZE_DELTA_USD,
                    initialCollateralDeltaAmount: LIT_COLLATERAL_AMOUNT_USDC,
                    triggerPrice: 0,
    ...
    /// @dev Executes a LIT/USD order like `_executeOrder`, but additionally mocks oracle
    ///  prices for keeper execution (LIT has no GMX on-chain feed, so the redirected
    ///  provider must return a valid 1e30-per-atom price). Afterwards re-establishes the
    ///  fallback environment: LIT's provider is redirected back to the GMX Chainlink
    ///  provider address and mocked to revert, and the fallback LIT price is re-mocked,
    ///  so valuation exercises the hardcoded fallback feed.
    ...
    // =========================================================================
    // Helpers — synthetic index valuation
    // =========================================================================
    ...
    function test_Nav_PendingOrder_StableWhilePending_RestoredAfterExecution() public {
        ...
        assertEq(
            navPending,
            navBefore,
            "NAV must be fully stable during pending (collateral + execution fee both tracked)"
        );
    ...
    function test_EGmxCallback_RecordsClaimableBalances_TwoTokens() public {
        ...
        bytes32 shortAmountKey = keccak256(
            abi.encode(GmxCallbackLib.CLAIMABLE_COLLATERAL_AMOUNT_KEY, market, shortToken, timeKey, pool)
        );
    ...
    /// @dev Remove any existing mapping for `selector` then register it to `agmxV2`.
    ///      Must be called while pranking the authority owner/whitelister. Needed because the
    ///      live Authority may already register the selectors (adapter deployed since the
    ///      pinned block), which would revert addMethod with SELECTOR_EXISTS_ERROR.
    ...
    function test_GetFallbackPrice_MatchesLiveFeedScaling() public {
        ...
        for (uint256 i; i < fallbackEntries.length; ++i) {
            ...
            uint256 gmxTokenDecimals = _gmxTokenDecimals(e.token);
            expectedPrices[i] =
                (uint256(answer) * 10 ** (60 - e.feedDecimals - uint8(gmxTokenDecimals))) /
                _FLOAT_PRECISION;
...
/// @dev Minimal GMX market token for fork parity tests. GMX's claimCollateral calls
///  `transferOut(token, receiver, amount)` on the market token; this mock simply
///  forwards the ERC-20 transfer from its own balance.
contract MockGmxMarketToken {
