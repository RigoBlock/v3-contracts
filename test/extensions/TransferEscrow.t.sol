...
contract EscrowWorkingForkTest is Test {
    ...
    function setUp() public {
        // Use Ethereum mainnet fork to access real USDC (pinned block, see ForkBlocks)
        vm.createSelectFork("mainnet", Constants.MAINNET_BLOCK);

        // Use real USDC (whitelisted on Across)
        testToken = Constants.ETH_USDC;
    ...
    function test_EscrowDeployment() public view {
        ...
        address expectedAddress = address(
            uint160(uint256(keccak256(abi.encodePacked(bytes1(0xff), pool, salt, bytecodeHash))))
        );
