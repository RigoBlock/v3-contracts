...
describe("Authority", async () => {
  ...
  describe("addMethod", async () => {
    it("should revert if caller not whitelister", async () => {
      const { authority, user2 } = await setupTests();
      const selector = "0xa694fc3a";
      const adapter = user2.address;
      await expect(authority.addMethod(selector, adapter)).to.be.revertedWith(
        "AUTHORITY_SENDER_NOT_WHITELISTER_ERROR",
      );
    });

    it("should revert if adapter not whitelisted", async () => {
      const { authority, user1, user2 } = await setupTests();
      const selector = "0xa694fc3a";
      const adapter = user2.address;
    ...
    it("should whitelist method", async () => {
      ...
      await expect(authority.addMethod(selector, adapter))
        .to.emit(authority, "WhitelistedMethod")
  ...
  describe("removeMethod", async () => {
    it("should revert if caller not whitelister", async () => {
      ...
      const adapter = user2.address;
      await expect(
        connect(authority, user2).removeMethod(selector, adapter),
    ...
    it("should remove method", async () => {
      ...
      const adapter = user2.address;
      await authority.setAdapter(adapter, true);
      await connect(authority, user2).addMethod(selector, adapter);
      await expect(connect(authority, user2).removeMethod(selector, adapter))
        .to.emit(authority, "RemovedMethod")
  ...
  describe("setWhitelister", async () => {
    it("should revert if caller not owner", async () => {
      const { authority, user2 } = await setupTests();
      await expect(
        connect(authority, user2).setWhitelister(user2.address, true),
    ...
    it("should revert with zero address input", async () => {
      ...
      await expect(
        authority.setWhitelister(ZeroAddress, true),
    ...
    it("should set whitelister", async () => {
      const { authority, user1, user2 } = await setupTests();
      await expect(authority.setWhitelister(user2.address, true))
        .to.emit(authority, "PermissionAdded")
    ...
    it("should remove whitelister", async () => {
      const { authority, user1, user2 } = await setupTests();
      await authority.setWhitelister(user2.address, true);
      await expect(authority.setWhitelister(user2.address, false))
        .to.emit(authority, "PermissionRemoved")
        .withArgs(user1.address, user2.address, Role.Whitelister);
    });
  });
  ...
  describe("setFactory", async () => {
    it("should revert if caller not owner", async () => {
      const { authority, user2 } = await setupTests();
      await expect(
        connect(authority, user2).setFactory(user2.address, true),
      ).to.be.revertedWith("OWNED_CALLER_IS_NOT_OWNER_ERROR");
    });

    it("should revert with zero address input", async () => {
      const { authority } = await setupTests();
      await expect(authority.setFactory(ZeroAddress, true)).to.be.revertedWith(
        "AUTHORITY_TARGET_NULL_ADDRESS_ERROR",
      );
    });
    ...
    it("should whitelist factory", async () => {
      const { authority, user1, user2 } = await setupTests();
      await expect(authority.setFactory(user2.address, true))
        .to.emit(authority, "PermissionAdded")
    ...
    it("should remove factory", async () => {
      const { authority, user1, user2 } = await setupTests();
      await authority.setFactory(user2.address, true);
      await expect(authority.setFactory(user2.address, false))
        .to.emit(authority, "PermissionRemoved")
        .withArgs(user1.address, user2.address, Role.Factory);
    });
  });
  ...
  describe("setAdapter", async () => {
    it("should revert if caller not owner", async () => {
      const { authority, user2 } = await setupTests();
      await expect(
        connect(authority, user2).setAdapter(user2.address, true),
      ).to.be.revertedWith("OWNED_CALLER_IS_NOT_OWNER_ERROR");
    });

    it("should revert with zero address input", async () => {
      const { authority } = await setupTests();
      await expect(authority.setAdapter(ZeroAddress, true)).to.be.revertedWith(
        "AUTHORITY_TARGET_NULL_ADDRESS_ERROR",
      );
    });
    ...
    it("should whitelist adapter", async () => {
      const { authority, user1, user2 } = await setupTests();
      await expect(authority.setAdapter(user2.address, true))
        .to.emit(authority, "PermissionAdded")
        .withArgs(user1.address, user2.address, Role.Adapter);
      await expect(authority.setAdapter(user2.address, false))
        .to.emit(authority, "PermissionRemoved")
        .withArgs(user1.address, user2.address, Role.Adapter);
    });
  });
