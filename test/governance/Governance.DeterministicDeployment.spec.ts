...
describe("Governance deterministic deployment", async () => {
  it("governance factory reproduces its deployed address on a fresh chain", async () => {
    ...
    expect(getCreate2Address(singletonFactory, ZeroHash, keccak256(bytecode))).to.eq(
      GOVERNANCE_FACTORY,
    );
  });
  ...
  it("governance proxy reproduces its deployed address on a fresh chain", async () => {
    ...
    expect(getCreate2Address(GOVERNANCE_FACTORY, salt, keccak256(bytecode))).to.eq(
      GOVERNANCE_PROXY,
    );
  });
});
