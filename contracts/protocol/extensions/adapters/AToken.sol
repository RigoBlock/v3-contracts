...
abstract contract AToken {
        ...
        external
    {
        ...
        require(
            _safeApprove(_token, _tokenTransferProxy, _amount),
    ...
    function resetMultipleAllowances(
        address _tokenTransferProxy,
    ...
    function _assertApprovedProxy(address _proxy) internal view {
        require(
            _getExchangesAuthorityInstance().isWhitelistedProxy(_proxy),
            "ATOKEN_NOT_APPROVED_PROXY_ERROR"
        );
    }

    /// @dev Finds the exchanges authority.
    /// @return Address of the exchanges authority.
    function _getExchangesAuthorityInstance()
        internal
        view
        returns (ExchangesAuthority)
    {
        return ExchangesAuthority(
            Authority(AUTHORITY).getExchangesAuthority()
        );
    }
    ...
    function _assertCallerIsPoolOwner() internal view {
        ...
        require(
            success && msg.sender == abi.decode(data, (address)),
            "CALLER_NOT_POOL_OWNER_ERROR"
        );
    }
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "ATOKEN_TOKEN_APPROVE_FAILED_ERROR"
        );
        return true;
    }
}
