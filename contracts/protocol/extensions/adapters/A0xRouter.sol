...
contract A0xRouter is IA0xRouter, IMinimumVersion, ReentrancyGuardTransient {
    ...
    function _checkActionsAllowed(address target, bytes calldata data) private pure {
        ...
        for (uint256 i; i < numActions; ++i) {
            ...
            // TRANSFER_FROM routes sell tokens from the pool into the Settler for internal
            // routing. In any legitimate 0x API swap, `recipient` is always the Settler itself.
            // Enforcing recipient == target closes the path where a pool owner crafts calldata
            // with recipient = attacker to drain ERC-20 tokens while bypassing share accounting.
