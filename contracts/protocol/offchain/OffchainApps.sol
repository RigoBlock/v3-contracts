...
/// @dev External positions are consolidating into a single view contract. As more apps are connected, can be split into multiple mixings.
/// @dev Future-proof as can route to dedicated extensions, should the size of the contract become too big.
abstract contract OffchainApps {
    ...
    /// @notice This method currently will return 0, as it accesses this contract's storage (which is empty), not the pool's.
    /// Further modifications will be implemented according to whether we implement it as a standalone contract, or as an adapter.
    /// When considering extension vs adapter, the adapter can be upgraded by gov, which is not a risk since it never modifies state.
    function _getOffchainAppTokenBalances(uint256 packedApplications, address /*pool*/) internal view returns (ExternalApp[] memory) {
    ...
    function _getUniV4PmBalances() private view returns (AppTokenBalance[] memory balances) {
    ...
    function _findCrossPrice(address token0, address token1) private view returns (uint160) {
        int24 twap0;
        int24 twap1;
