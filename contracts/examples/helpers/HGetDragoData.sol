...
contract HGetDragoData {
    ...
    function queryDataFromId(address _dragoRegistry, uint256 _dragoId) external view returns (DragoData memory dragoData) {
        ...
        (dragoData) = queryDataInternal(drago);
    ...
    function queryDataFromAddress(address _dragoRegistry, address _dragoAddress) external view returns (DragoData memory dragoData) {
        ...
        (dragoData) = queryDataInternal(_dragoAddress);
    ...
    function queryMultiDataFromId(address _dragoRegistry, uint256[] calldata _dragoIds) external view returns (DragoData[] memory) {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            (dragoData[i]) = queryDataInternal(dragos[i]);
    ...
    /// @return Arrays of structs of data and related address of a drago.
    function queryMultiDataFromAddress(address _dragoRegistry, address[] calldata _dragoAddresses)
        external
        view
    ...
    /// @return dragoData Structs of data.
    function queryDataInternal(address _drago) internal view returns (DragoData memory dragoData) {
        DragoFace dragoInstance = DragoFace(_drago);
        (dragoData.name, dragoData.symbol, dragoData.sellPrice, dragoData.buyPrice) = dragoInstance.getData();
        (
            dragoData.owner,
